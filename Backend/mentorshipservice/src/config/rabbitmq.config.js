...
export const connectRabbitMQ = async (retries = 5, delay = 5000) => {
  for (let i = 1; i <= retries; i++) {
    try {
      ...
      logger.info("RabbitMQ Connected successfully to mentorshipservice");

      connection.on("error", (err) => {
        logger.error(`RabbitMQ connection error in mentorshipservice: ${err.message}`);
      });

      connection.on("close", () => {
        logger.warn("RabbitMQ connection closed in mentorshipservice. Reconnecting in 5s...");
